<?php
	...
	$addrs = $rows_user['addrs'];
	$age = $rows_user['age'];
	$sex = $rows_user['sex'];
	$semester = $rows_user['semester'];
	$dprtmnt = $rows_user['dprtmnt'];
	?>
 <!DOCTYPE html>
 <html lang="en">
 								...
 								<div class="panel-body text-center">
 									<div>
 										<?php if ($photo != '') {
												...
												echo '<img src="../admin/dist/img/user4.png" style="width:100%">';
											} ?>
 									</div>
 										...
 										<div class="row">
											...
											<div class="col-md-6">
 												<div class="form-group">
 													...
 													<input type="text" class="form-control" placeholder="Age" name="age" value="<?php echo $age ?>" min="16" max="100" required>
 												</div>
 											</div>
											<div class="col-md-12">
 												<div class="form-group">
                                             ...
                                             <div class="col-md-6">
 												<div class="form-group">
 													...
 													<input list="dprtmnt" required class="form-control" name="dprtmnt" placeholder="Search" value="<?php echo $dprtmnt; ?>">
                                                    <datalist id="dprtmnt">
                                                        <option value="">Select</option> 
                                                         <?php
                                                       $result = $db->prepare("SELECT distinct(dprtmnt) FROM users where user_type='Customer'");
                                                            $result->execute();
                                                            $row_count =  $result->rowcount();
                                                            for($i=0; $rows = $result->fetch(); $i++)
                                                            {
                                                            echo '<option>'.$rows['dprtmnt'].'</option>';
                                                            }
                                                        ?>	                                         					
                                                    </datalist>
 												</div>
                                            ...
                                            <div class="col-md-6">
 												<div class="form-group">
 													...
 													<input list="semester" required class="form-control" name="semester" placeholder="Search" value="<?php echo $semester; ?>">
                                                    <datalist id="semester">
                                                        <option value="">Select</option> 
                                                         <?php
                                                     $result = $db->prepare("SELECT distinct(semester) FROM users where user_type='Customer'");
                                                            $result->execute();
                                                            $row_count =  $result->rowcount();
                                                            for($i=0; $rows = $result->fetch(); $i++)
                                                            {
